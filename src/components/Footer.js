      ...
      <div style={{ borderBottom:'1px solid rgba(255,255,255,0.07)', padding:'48px 24px' }}>
        <div className="container">
          <div className="cta-strip">
            <div>
              <div className="subheading" style={{ fontSize:'1.7rem', color:'white', lineHeight:1, marginBottom:6 }}>READY TO MOVE YOUR CARGO?</div>
        ...
        <div className="container">
          <div className="grid-footer">
            <div>
              ...
              <p style={{ color:'rgba(255,255,255,0.45)', fontSize:'0.85rem', lineHeight:1.7, marginBottom:20 }}>Expertly managing your supply chain from start to finish — delivering goods safely, on time, and at the best rates.</p>
              <div style={{ display:'flex', gap:8 }}>
                {[Globe,Mail,Phone].map((Icon,i)=>(
                  <div key={i} style={{ width:34, height:34, borderRadius:8, background:'rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}><Icon size={14} color="rgba(255,255,255,0.45)"/></div>
                ))}
              </div>
            </div>
            <div>
              <div className="subheading" style={{ fontSize:'0.72rem', color:'#F5C518', letterSpacing:'0.14em', textTransform:'uppercase', marginBottom:16 }}>Services</div>
              {services.map(s=><div key={s} style={{ marginBottom:8 }}><a href="#services" style={{ color:'rgba(255,255,255,0.45)', fontSize:'0.85rem' }}>{s}</a></div>)}
            </div>
            <div>
              <div className="subheading" style={{ fontSize:'0.72rem', color:'#F5C518', letterSpacing:'0.14em', textTransform:'uppercase', marginBottom:16 }}>Company</div>
              {company.map(c=><div key={c} style={{ marginBottom:8 }}><a href="#" style={{ color:'rgba(255,255,255,0.45)', fontSize:'0.85rem' }}>{c}</a></div>)}
            </div>
            <div>
              <div className="subheading" style={{ fontSize:'0.72rem', color:'#F5C518', letterSpacing:'0.14em', textTransform:'uppercase', marginBottom:16 }}>Contact</div>
              {[
                ...
                <div key={item.val} style={{ display:'flex', alignItems:'flex-start', gap:10, marginBottom:14 }}>
                  ...
                  {item.href?<a href={item.href} style={{ color:'rgba(255,255,255,0.45)', fontSize:'0.82rem', lineHeight:1.5 }}>{item.val}</a>:<span style={{ color:'rgba(255,255,255,0.45)', fontSize:'0.82rem', lineHeight:1.5, whiteSpace:'pre-line' }}>{item.val}</span>}
                </div>
              ))}
