    ...
    <section id="industries" style={{ background: '#fff', padding: '96px 24px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 60 }}>
          <span className="section-tag" style={{ background: 'rgba(45,15,107,0.07)', color: '#2D0F6B' }}>Industries We Serve</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }} className="stack-mobile">
          {industries.map(ind => (
            <div key={ind.title} style={{ borderRadius: 18, overflow: 'hidden', height: 260, position: 'relative', cursor: 'pointer' }}
              onMouseEnter={e => { e.currentTarget.querySelector('img').style.transform = 'scale(1.08)'; e.currentTarget.querySelector('.overlay').style.opacity = '1'; e.currentTarget.querySelector('.desc').style.opacity = '1'; e.currentTarget.querySelector('.desc').style.transform = 'translateY(0)'; }}
              onMouseLeave={e => { e.currentTarget.querySelector('img').style.transform = 'scale(1)'; e.currentTarget.querySelector('.overlay').style.opacity = '0'; e.currentTarget.querySelector('.desc').style.opacity = '0'; e.currentTarget.querySelector('.desc').style.transform = 'translateY(8px)'; }}
            >
              ...
              <div className="overlay" style={{ position: 'absolute', inset: 0, background: 'rgba(45,15,107,0.45)', opacity: 0, transition: 'opacity 0.3s' }} />
              <div style={{ position: 'absolute', inset: 0, padding: 24, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                ...
                <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 800, fontSize: '1.2rem', color: 'white', letterSpacing: '0.03em', marginBottom: 4 }}>{ind.title}</div>
                <div className="desc" style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.75)', lineHeight: 1.5, opacity: 0, transform: 'translateY(8px)', transition: 'opacity 0.3s, transform 0.3s' }}>{ind.desc}</div>
              </div>
              {/* Gold top accent on hover — done via pseudo, skip in inline; use bottom line instead */}
