...
export default function WhyUs() {
  return (
    <section id="why-us" className="section" style={{ background:'linear-gradient(135deg,#1E0A4F 0%,#2D0F6B 100%)', position:'relative', overflow:'hidden' }}>
      ...
      <div className="container" style={{ position:'relative' }}>
        <div className="grid-2">
          ...
          <div className="desktop-only" style={{ position:'relative' }}>
            <div style={{ borderRadius:20, overflow:'hidden', height:500, boxShadow:'0 30px 80px rgba(0,0,0,0.4)', position:'relative' }}>
              <img src="https://images.unsplash.com/photo-1494412651409-8963ce7935a7?w=700&q=80" alt="Freight" />
            ...
            <div style={{ position:'absolute', top:20, left:20, background:'rgba(26,138,60,0.92)', border:'1px solid #22A84A', borderRadius:10, padding:'7px 14px', fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:'0.72rem', color:'white', letterSpacing:'0.1em', textTransform:'uppercase' }}>✓ Licensed & Insured</div>
          </div>
          {/* Content */}
          <div>
              ...
              {reasons.map(r => {
                ...
                return (
                  <div key={r.title} className="card" style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:14, padding:'16px 18px', display:'flex', gap:12, alignItems:'flex-start' }}>
                    <div style={{ width:36, height:36, borderRadius:10, background:'rgba(245,197,24,0.12)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><Icon size={16} color="#F5C518" /></div>
                    <div>
                      <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'0.95rem', color:'white', marginBottom:4 }}>{r.title}</div>
